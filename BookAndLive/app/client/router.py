...
async def client_by_id(client_id: str,
    ...
    if (client := await mongo_repository.get_client(client_id)) is not None:
        return {"client": client}
...
async def client_update(client_id: str,
    ...
    if (updated_client := await mongo_repository.update_client(client_id, client_upd)) is not None:
        await es_repository.update(client_id, client_upd)
...
async def delete_client(client_id: str,
    ...
    if (deleted_client := await mongo_repository.delete_client(client_id)) is not None:
        await es_repository.delete(client_id)
