...
async def get_all_reservations(mongo_repository: ReservationMongoRepository
...
async def reservation_update(reservation_id: str,
                             ...
                             mongo_repository: ReservationMongoRepository = Depends(
                                 ReservationMongoRepository.mongo_reservation_factory),
                             mongo_repository_client: ClientMongoRepository = Depends(
                                 ClientMongoRepository.mongo_client_factory),
                             mongo_repository_room: RoomMongoRepository = Depends(
                                 RoomMongoRepository.mongo_room_factory),
                             es_repository: ReservationEsRepository = Depends(
                                 ReservationEsRepository.es_reservation_factory)):
    if not ObjectId.is_valid(reservation_id):
        raise HTTPException(status_code=400, detail='Bad Request')
