...
# thousands thanks to Andre Kamman https://www.youtube.com/watch?v=x3yf4Tr08BU&ab_channel=Andr%C3%A9Kamman
...
parser.add_argument("-p", "--progressindicatorvalue", help="Shows nr of rows imported for larger files", type=int,
                    default=10000000)
...
tables = {
    ...
    "Posts": ["Id", "PostTypeId", "ParentId", "AcceptedAnswerId", "CreationDate", "Score", "ViewCount", "Body",
...
for subfolder in subfolders:
    ...
    if has_validfiles(subfolder, stackexchangefiles):
        ...
        if not os.path.isdir(
                "{outputdir}\\{subfolder}".format(outputdir=outputdir, subfolder=os.path.basename(subfolder))):
            ...
            for file in stackexchangefiles:
                if os.path.isfile("{subfolder}\\{file}".format(subfolder=subfolder, file=file)):
                    write_csv_from_xml("{subfolder}\\{sourcefile}".format(subfolder=subfolder, sourcefile=file),
                                       Path(file).stem, tables[Path(file).stem],
                                       "{destinationfolder}\\{subfolder}\\{tablename}.csv".format(
                                           destinationfolder=outputdir, subfolder=os.path.basename(subfolder),
